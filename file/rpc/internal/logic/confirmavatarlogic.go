...
type ConfirmAvatarUploadLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewConfirmAvatarUploadLogic(ctx context.Context, svcCtx *svc.ServiceContext) *ConfirmAvatarUploadLogic {
	return &ConfirmAvatarUploadLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
...
func (l *ConfirmAvatarUploadLogic) ConfirmAvatarUpload(in *file.ConfirmAvatarUploadRequest) (*file.ConfirmUploadResponse, error) {
	...
	logx.Infof("用户 %d 上传头像成功: %s (%d bytes)", userId, in.Filename, in.FileSize)
...
func (l *ConfirmAvatarUploadLogic) buildDownloadURL(objectKey string) string {
